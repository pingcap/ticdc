...
func CfInfoToAPIModel(
	...
	if info.Config != nil {
		maskedConfig := info.Config.Clone()
		maskedConfig.MaskSensitiveData()
		replicaConfig = ToAPIReplicaConfig(maskedConfig)
	}
...
func (h *OpenAPIV2) UpdateChangefeed(c *gin.Context) {
	...
	if updateCfConfig.SinkURI != "" {
		oldCfInfo.SinkURI, sinkURIUpdated = restoreMaskedSinkURI(updateCfConfig.SinkURI, oldCfInfo.SinkURI)
	}
...
func restoreMaskedSinkURI(updated, original string) (string, bool) {
	restored := restoreMaskedURI(updated, original)
...
func restoreMaskedURI(updated, original string) string {
	...
	if !strings.EqualFold(updatedURI.Scheme, originalURI.Scheme) ||
		...
		updatedURI.User.Username() != originalURI.User.Username() {
		return updated
	}
	...
	if updatedHasPassword && updatedPassword == "xxxxx" && originalHasPassword {
		updatedURI.User = url.UserPassword(updatedURI.User.Username(), originalPassword)
	}
...
func restoreMaskedSensitiveData(updated, original *config.ReplicaConfig) {
	...
	if updated.Sink != nil && original.Sink != nil && masked.Sink != nil {
		restoreMaskedURIString(&updated.Sink.SchemaRegistry, masked.Sink.SchemaRegistry, original.Sink.SchemaRegistry)

		if updated.Sink.KafkaConfig != nil && original.Sink.KafkaConfig != nil && masked.Sink.KafkaConfig != nil {
			updatedKafka := updated.Sink.KafkaConfig
			originalKafka := original.Sink.KafkaConfig
			maskedKafka := masked.Sink.KafkaConfig
			...
			restoreMaskedURIString(&updatedKafka.SASLOAuthTokenURL, maskedKafka.SASLOAuthTokenURL, originalKafka.SASLOAuthTokenURL)
		...
		if updated.Sink.PulsarConfig != nil && original.Sink.PulsarConfig != nil && masked.Sink.PulsarConfig != nil {
			updatedPulsar := updated.Sink.PulsarConfig
			originalPulsar := original.Sink.PulsarConfig
			maskedPulsar := masked.Sink.PulsarConfig
	...
	if updated.Consistent != nil && original.Consistent != nil && masked.Consistent != nil {
		restoreMaskedURIString(&updated.Consistent.Storage, masked.Consistent.Storage, original.Consistent.Storage)
	}
}

func restoreMaskedURIString(updated **string, masked, original *string) {
	restoreMaskedString(updated, masked, original)
	if updated == nil || *updated == nil || original == nil {
		return
	}
	value := restoreMaskedURI(**updated, *original)
	*updated = &value
}
