...
func TestSetRedactMode(t *testing.T) {
	tests := []struct {
		...
		// Invalid transitions - same level or decreasing restriction
		{
			name:           "OFF -> OFF (rejected, same level)",
			initialMode:    perrors.RedactLogDisable,
		...
		{
			name:           "MARKER -> MARKER (rejected, same level)",
			initialMode:    perrors.RedactLogMarker,
			requestBody:    RedactModeReq{Mode: "marker"},
			expectedStatus: http.StatusOK,
		...
		{
			name:           "ON -> ON (rejected, same level)",
		...
		// Edge cases
		{
			...
			expectedMode:   perrors.RedactLogDisable, // No change
			expectError:    true,
			errorContains:  "invalid redaction mode",
		},
		{
			name:           "invalid JSON",
			initialMode:    perrors.RedactLogDisable,
		...
		{
			name:           "empty string mode -> ON (success, handles empty)",
			initialMode:    "", // Empty string, will default to OFF
			requestBody:    RedactModeReq{Mode: "on"},
			expectedStatus: http.StatusOK,
			expectedMode:   perrors.RedactLogEnable,
			expectError:    false,
