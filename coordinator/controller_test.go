...
func TestHandleSingleMaintainerStatusCleansUpUnexpectedMaintainer(t *testing.T) {
	ctrl := gomock.NewController(t)
	backend := mock_changefeed.NewMockBackend(ctrl)
	changefeedDB := changefeed.NewChangefeedDB(1216)
	...
	cf := changefeed.NewChangefeed(cfID, info, 1, true)
	changefeedDB.AddReplicatingMaintainer(cf, "node1")

	// Report a working status from an unexpected node. The controller should ignore it
	// and send a best-effort remove request to prevent duplicate maintainers.
	...
	controller.handleSingleMaintainerStatus("node2", status, cfID)

	select {
	case msg := <-mc.GetMessageChannel():
		require.Equal(t, node.ID("node2"), msg.To)
		require.Equal(t, messaging.TypeRemoveMaintainerRequest, msg.Type)
		req, ok := msg.Message[0].(*heartbeatpb.RemoveMaintainerRequest)
		require.True(t, ok)
		require.True(t, req.Cascade)
		require.False(t, req.Removed)
		require.Equal(t, uint32(1), req.KeyspaceId)
		require.Equal(t, cfID.ToPB(), req.Id)
	case <-time.After(time.Second):
		require.FailNow(t, "expected a cleanup remove maintainer request")
	}
}

func TestHandleSingleMaintainerStatusCleansUpMaintainerForPausedChangefeed(t *testing.T) {
	ctrl := gomock.NewController(t)
	backend := mock_changefeed.NewMockBackend(ctrl)
	changefeedDB := changefeed.NewChangefeedDB(1216)
