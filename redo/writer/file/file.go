...
func (w *fileWriter) Write(rawData []byte) (int, error) {
	w.Lock()
	defer w.Unlock()
...
func (w *fileWriter) AdvanceTs(commitTs uint64) {
	w.eventCommitTS.Store(commitTs)
}

func (w *fileWriter) writeUint64(n uint64, buf []byte) error {
...
func (w *fileWriter) IsRunning() bool {
	return w.running.Load()
}

func (w *fileWriter) syncWrite(event writer.RedoEvent) error {
	rl := event.ToRedoLog()
...
func (w *fileWriter) encode(ctx context.Context) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case e := <-w.inputCh:
...
func (w *fileWriter) newPageWriter() error {
...
func (w *fileWriter) flushAndRotateFile() error {
	if w.file == nil {
		return nil
	}
...
func (w *fileWriter) writeToS3(ctx context.Context, name string) error {
