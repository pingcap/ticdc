...
> 背景与根因分析见：`storage-sink-slow.md`。吞吐/稳定性修复方案见：`design.md`（in-flight budget）。
...
- 一旦上游允许持续投喂（例如开启 `await=false`），无界队列会把压力“搬进进程内存”，从 dynstream 的 quota 体系中逃逸（需要额外约束，见 `design.md`）。
...
1. **修复吞吐节拍化：允许同 dispatcher 多批次 in-flight，同时把背压留在 dynstream**
   - 采用 `design.md` 的 Option A（dispatcher in-flight budget + global wake 协调器）。
   - 目标：让 storage sink 在一个 flush 窗口内持续有数据可写，并避免把 backlog 无界搬进 sink 内部。
...
3. **长期状态回收**
   - `FilePathGenerator.fileIndex/versionMap` 随“历史触达集合”增长的问题，在 total 极大且 churn 大时可能需要 TTL 回收（见 `design.md` Future Work）。
...
- 结合 `design.md` 的 in-flight budget 指标（global in-flight bytes、blocked dispatchers）判断是“预算过大”还是“外部存储变慢”。
