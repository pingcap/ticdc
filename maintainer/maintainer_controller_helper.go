...
func (c *Controller) moveTable(tableId int64, targetNode node.ID) error {
	...
	replications := c.spanManager.GetTasksByTableID(tableId)
...
func (c *Controller) splitTableByRegionCount(tableID int64) error {
	if !c.spanManager.GetReplicationDB().IsTableExists(tableID) {
	...
	primaryOp := operator.NewMergeSplitDispatcherOperator(c.spanManager.GetReplicationDB(), primaryID, replications[randomIdx], replications, splitTableSpans, nil)
	for _, replicaSet := range replications {
		var op *operator.MergeSplitDispatcherOperator
		if replicaSet.ID == primaryID {
			op = primaryOp
		} else {
			op = operator.NewMergeSplitDispatcherOperator(c.spanManager.GetReplicationDB(), primaryID, replicaSet, nil, nil, primaryOp.GetOnFinished())
		}
		c.operatorController.AddOperator(op)
	}
...
func (c *Controller) checkParams(tableId int64, targetNode node.ID) error {
	...
	nodes := c.spanManager.GetAllNodes()
	hasNode := false
