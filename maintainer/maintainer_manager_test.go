...
func TestMaintainerSchedulesNodeChanges(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.replicationDB.GetSchedulingSize() == 4
	}, 20*time.Second, 200*time.Millisecond)
	require.Equal(t, 4,
		maintainer.controllerManager.controller.GetTaskSizeByNodeID(selfNode.ID))
	...
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.replicationDB.GetReplicatingSize() == 4
	}, 20*time.Second, 200*time.Millisecond)
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(selfNode.ID) == 1
	}, 20*time.Second, 200*time.Millisecond)
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(node2.ID) == 1
	}, 20*time.Second, 200*time.Millisecond)
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(node3.ID) == 1
	}, 20*time.Second, 200*time.Millisecond)
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(node4.ID) == 1
	}, 20*time.Second, 200*time.Millisecond)
	...
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(selfNode.ID) == 2
	}, 20*time.Second, 200*time.Millisecond)
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(node2.ID) == 2
	}, 20*time.Second, 200*time.Millisecond)

	log.Info("Pass case 3: Remove 2 nodes")

	// Case 4: Remove 2 tables
	maintainer.controllerManager.RemoveTasksByTableIDs(false, 2, 3)
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.replicationDB.GetReplicatingSize() == 2
	}, 20*time.Second, 200*time.Millisecond)
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(selfNode.ID) == 1
	}, 20*time.Second, 200*time.Millisecond)
	...
	// Case 5: Add 2 tables
	maintainer.controllerManager.controller.AddNewTable(commonEvent.Table{
		SchemaID: 1,
		TableID:  5,
	}, 3)
	maintainer.controllerManager.controller.AddNewTable(commonEvent.Table{
...
func TestMaintainerBootstrapWithTablesReported(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return maintainer.controllerManager.controller.GetTaskSizeByNodeID(selfNode.ID) == 4
	}, 20*time.Second, 200*time.Millisecond)

	require.Len(t, remotedIds, 2)
	foundSize := 0
	hasDDLDispatcher := false
	for _, stm := range maintainer.controllerManager.controller.replicationDB.GetReplicating() {
