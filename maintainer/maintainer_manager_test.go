...
func TestMaintainerSchedulesNodeChanges(t *testing.T) {
	...
	mc := messaging.NewMessageCenter(ctx, selfNode.ID, config.NewDefaultMessageCenterConfig(), nil)
	mc.Run(ctx)
	defer mc.Close()
	...
	mc2 := messaging.NewMessageCenter(ctx, node2.ID, config.NewDefaultMessageCenterConfig(), nil)
	mc2.Run(ctx)
	defer mc2.Close()

	node3 := node.NewInfo("127.0.0.1:8500", "")
	mc3 := messaging.NewMessageCenter(ctx, node3.ID, config.NewDefaultMessageCenterConfig(), nil)
	mc3.Run(ctx)
	defer mc3.Close()

	node4 := node.NewInfo("127.0.0.1:8600", "")
	mc4 := messaging.NewMessageCenter(ctx, node4.ID, config.NewDefaultMessageCenterConfig(), nil)
	mc4.Run(ctx)
	defer mc4.Close()
