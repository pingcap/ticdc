...
func TestSchedule(t *testing.T) {
	...
	for _, span := range controller.spanManager.GetTasksBySchemaID(1) {
		if op := controller.operatorController.GetOperator(span.ID); op != nil {
			op.Start()
		}
	}
	require.Equal(t, 1, controller.spanManager.GetAbsentSize())
	require.Equal(t, 3, controller.spanManager.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 3, controller.spanManager.GetTaskSizeByNodeID("node2"))
	require.Equal(t, 3, controller.spanManager.GetTaskSizeByNodeID("node3"))
}
...
func TestRemoveAbsentTask(t *testing.T) {
	...
	require.Equal(t, 1, controller.spanManager.GetAbsentSize())
	controller.operatorController.RemoveAll()
	require.Equal(t, 0, controller.spanManager.GetAbsentSize())
}
...
func TestBalanceGlobalEven(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		s.spanManager.AddReplicatingSpan(spanReplica)
	}
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 0, s.operatorController.OperatorSize())
	require.Equal(t, 100, s.spanManager.GetReplicatingSize())
	require.Equal(t, 100, s.spanManager.GetTaskSizeByNodeID(nodeID))

	// add new node
	nodeManager.GetAliveNodes()["node2"] = &node.Info{ID: "node2"}
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 50, s.operatorController.OperatorSize())
	require.Equal(t, 50, s.spanManager.GetSchedulingSize())
	require.Equal(t, 50, s.spanManager.GetReplicatingSize())
	for _, span := range s.spanManager.GetTasksBySchemaID(1) {
	...
	require.Equal(t, 100, s.spanManager.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 0, s.spanManager.GetTaskSizeByNodeID("node2"))

	// remove the node2
	delete(nodeManager.GetAliveNodes(), "node2")
	s.operatorController.OnNodeRemoved(nodeID)
	for _, span := range s.spanManager.GetTasksBySchemaID(1) {
		if op := s.operatorController.GetOperator(span.ID); op != nil {
			msg := op.Schedule()
	...
	require.Equal(t, 0, s.spanManager.GetSchedulingSize())
...
func TestBalanceGlobalUneven(t *testing.T) {
	...
	require.Equal(t, 0, s.operatorController.OperatorSize())
	require.Equal(t, 100, s.spanManager.GetReplicatingSize())
	require.Equal(t, 50, s.spanManager.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 50, s.spanManager.GetTaskSizeByNodeID("node2"))

	// add new node
	nodeManager.GetAliveNodes()["node3"] = &node.Info{ID: "node3"}
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 33, s.operatorController.OperatorSize())
	require.Equal(t, 33, s.spanManager.GetSchedulingSize())
	require.Equal(t, 67, s.spanManager.GetReplicatingSize())
	for _, span := range s.spanManager.GetTasksBySchemaID(1) {
		if op := s.operatorController.GetOperator(span.ID); op != nil {
			_, ok := op.(*operator.MoveDispatcherOperator)
			require.True(t, ok)
	...
	s.operatorController.OnNodeRemoved(node.ID("node3"))
	for _, span := range s.spanManager.GetTasksBySchemaID(1) {
		if op := s.operatorController.GetOperator(span.ID); op != nil {
			msg := op.Schedule()
			require.NotNil(t, msg)
...
func TestBalance(t *testing.T) {
	...
	// remove the node2
	delete(nodeManager.GetAliveNodes(), "node2")
	s.operatorController.OnNodeRemoved(node.ID("node2"))
	for _, span := range s.spanManager.GetTasksBySchemaID(1) {
		if op := s.operatorController.GetOperator(span.ID); op != nil {
			msg := op.Schedule()
			require.NotNil(t, msg)
...
func TestStoppedWhenMoving(t *testing.T) {
	...
	require.Equal(t, 2, s.spanManager.GetReplicatingSize())
	require.Equal(t, 2, s.spanManager.GetTaskSizeByNodeID("node1"))
	// add new node
	nodeManager.GetAliveNodes()["node2"] = &node.Info{ID: "node2"}
	require.Equal(t, 0, s.spanManager.GetAbsentSize())
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 1, s.operatorController.OperatorSize())
	require.Equal(t, 1, s.spanManager.GetSchedulingSize())
	require.Equal(t, 1, s.spanManager.GetReplicatingSize())
	require.Equal(t, 2, s.spanManager.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 0, s.spanManager.GetTaskSizeByNodeID("node2"))

	s.operatorController.OnNodeRemoved(node.ID("node2"))
	s.operatorController.OnNodeRemoved(node.ID("node1"))
	require.Equal(t, 0, s.spanManager.GetSchedulingSize())
	// changed to absent status
	require.Equal(t, 2, s.spanManager.GetAbsentSize())
	require.Equal(t, 0, s.spanManager.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 0, s.spanManager.GetTaskSizeByNodeID("node2"))
}
...
func TestFinishBootstrap(t *testing.T) {
	...
	require.Equal(t, 1, s.spanManager.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 1, s.spanManager.GetReplicatingSize())
	require.Equal(t, 0, s.spanManager.GetSchedulingSize())
	require.NotNil(t, s.spanManager.GetTaskByID(dispatcherID2))
...
func TestBalanceUnEvenTask(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		dispatcherID := common.NewDispatcherID()
		spanReplica := replica.NewSpanReplication(cfID, dispatcherID, 1, span, 1)
		s.spanManager.AddAbsentReplicaSet(spanReplica)
	}
	...
	require.Equal(t, 4, s.spanManager.GetReplicatingSize())
	require.Equal(t, 0, s.spanManager.GetSchedulingSize())
	...
	nodeManager.GetAliveNodes()["node3"] = &node.Info{ID: "node3"}
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 3, s.spanManager.GetReplicatingSize())
	require.Equal(t, 1, s.operatorController.OperatorSize())
	// still on the primary node
	require.Equal(t, 2, s.spanManager.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 2, s.spanManager.GetTaskSizeByNodeID("node2"))
	require.Equal(t, 0, s.spanManager.GetTaskSizeByNodeID("node3"))
	...
	require.Equal(t, 1, s.spanManager.GetTaskSizeByNodeID("node3"))
}
...
func TestSplitTableWhenBootstrapFinished(t *testing.T) {
	...
	require.Equal(t, 4, s.spanManager.GetAbsentSize())
...
func TestDynamicSplitTableBasic(t *testing.T) {
	...
	replicas := s.spanManager.GetReplicating()
	require.Equal(t, 2, s.spanManager.GetReplicatingSize())

	for _, task := range replicas {
		for cnt := 0; cnt < replica.HotSpanScoreThreshold; cnt++ {
			s.spanManager.UpdateStatus(task, &heartbeatpb.TableSpanStatus{
				ID:                 task.ID.ToPB(),
				ComponentStatus:    heartbeatpb.ComponentState_Working,
				CheckpointTs:       10,
	...
	require.Equal(t, 2, s.spanManager.GetSchedulingSize())
...
func TestDynamicMergeAndSplitTable(t *testing.T) {
	...
	replicas := s.spanManager.GetReplicating()
	require.Equal(t, totalTables*3-1, s.spanManager.GetReplicatingSize())
	...
	require.Equal(t, 7, s.spanManager.GetAbsentSize())
}
...
func TestDynamicMergeTableBasic(t *testing.T) {
	...
	victimExpected := 2
	replicas := s.spanManager.GetReplicating()
	require.Equal(t, expected+victimExpected, s.spanManager.GetReplicatingSize())
	...
	require.Equal(t, victimExpected, s.spanManager.GetReplicatingSize())
	require.Equal(t, expected, s.spanManager.GetSchedulingSize())
	...
	require.Equal(t, totalTables-1, s.spanManager.GetAbsentSize())
	...
	require.Equal(t, 0, s.spanManager.GetReplicatingSize())
	...
	require.Equal(t, totalTables, s.spanManager.GetAbsentSize())
}
