...
type Controller struct {
	...
	// maintainerCommittedCheckpointTs is the monotonic checkpoint baseline shared by
	// all spans in this controller. Any newly created dispatcher must not start below it.
...
func NewController(
	...
	c := &Controller{
		...
		maintainerCommittedCheckpointTs: atomic.NewUint64(0),
...
func (c *Controller) AdvanceMaintainerCommittedCheckpointTs(ts uint64) {
	...
	util.MustCompareAndMonotonicIncrease(c.maintainerCommittedCheckpointTs, ts)
}
