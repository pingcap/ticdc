...
func TestSplitRegionsSingleRegion(t *testing.T) {
	t.Parallel()
	re := require.New(t)
	...
	re.EqualValues(1, info.RegionCounts[0])
	re.Len(info.Weights, 1)
	re.EqualValues(101, info.Weights[0]) // 100 + regionWrittenKeyBase(1)
	re.Len(info.Spans, 1)
	re.EqualValues(startKeys[2], info.Spans[0].StartKey)
	re.EqualValues(endKeys[2], info.Spans[0].EndKey)
	re.EqualValues(int64(1), info.Spans[0].TableID)
}

func TestSplitRegionsSpansNumOne(t *testing.T) {
	t.Parallel()
	re := require.New(t)
	...
	re.EqualValues(3, info.RegionCounts[0])
	re.Len(info.Weights, 1)
	re.EqualValues(603, info.Weights[0]) // 100+200+300 + 3*regionWrittenKeyBase
	re.Len(info.Spans, 1)
	re.EqualValues(startKeys[2], info.Spans[0].StartKey)
	re.EqualValues(endKeys[4], info.Spans[0].EndKey)
}

func TestSplitRegionsPDAPIFailure(t *testing.T) {
	t.Parallel()
	re := require.New(t)
