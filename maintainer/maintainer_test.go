...
func TestMaintainerPushEventBuffersHeartbeatWatermarkStrictly(t *testing.T) {
	from := node.ID("node-1")
	changefeedID := common.NewChangeFeedIDWithName("test", common.DefaultKeyspaceName)
	m := &Maintainer{
		changefeedID:          changefeedID,
	...
	newHeartbeat := func(watermark, redoWatermark *heartbeatpb.Watermark) *heartbeatpb.HeartBeatRequest {
		return &heartbeatpb.HeartBeatRequest{
			ChangefeedID:  changefeedID.ToPB(),
			Watermark:     watermark,
			RedoWatermark: redoWatermark,
		}
	}
	pushHeartbeat := func(req *heartbeatpb.HeartBeatRequest) {
		t.Helper()
		require.True(t, m.pushEvent(&Event{
			changefeedID: m.changefeedID,
			eventType:    EventMessage,
	...
	pushHeartbeat(newHeartbeat(
		&heartbeatpb.Watermark{CheckpointTs: 100, ResolvedTs: 100, Seq: 10, LastSyncedTs: 50},
		&heartbeatpb.Watermark{CheckpointTs: 90, ResolvedTs: 90, Seq: 8, LastSyncedTs: 70},
	))
	pushHeartbeat(newHeartbeat(
		&heartbeatpb.Watermark{CheckpointTs: 100, ResolvedTs: 80, Seq: 10, LastSyncedTs: 60},
		&heartbeatpb.Watermark{CheckpointTs: 90, ResolvedTs: 70, Seq: 8, LastSyncedTs: 80},
	))

	events := m.takeBufferedStatusRequestEvents()
	require.Len(t, events, 1)
	req := events[0].message.Message[0].(*heartbeatpb.HeartBeatRequest)
	require.Equal(t, uint64(100), req.Watermark.CheckpointTs)
	require.Equal(t, uint64(100), req.Watermark.ResolvedTs)
	require.Equal(t, uint64(10), req.Watermark.Seq)
	require.Equal(t, uint64(60), req.Watermark.LastSyncedTs)
	require.Equal(t, uint64(90), req.RedoWatermark.CheckpointTs)
	require.Equal(t, uint64(90), req.RedoWatermark.ResolvedTs)
	require.Equal(t, uint64(8), req.RedoWatermark.Seq)
	require.Equal(t, uint64(80), req.RedoWatermark.LastSyncedTs)
}
