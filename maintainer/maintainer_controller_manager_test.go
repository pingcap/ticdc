...
func TestRemoveAbsentTask(t *testing.T) {
	...
	controllerManager := NewControllerManager(cfID, 1, nil, nil, nil, nil, ddlSpan, nil, 9, time.Minute)
	controllerManager.controller.AddNewTable(commonEvent.Table{
		SchemaID: 1,
		TableID:  int64(1),
	}, 1)
	require.Equal(t, 1, controllerManager.controller.replicationDB.GetAbsentSize())
	controllerManager.operatorController.RemoveAllTasks()
	require.Equal(t, 0, controllerManager.controller.replicationDB.GetAbsentSize())
}
...
func TestBalanceGlobalEven(t *testing.T) {
	...
	s := NewControllerManager(cfID, 1, nil, nil, nil, nil, ddlSpan, nil, 1000, 0)
	...
	require.Equal(t, 100, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, 100, s.controller.replicationDB.GetTaskSizeByNodeID(nodeID))

	// add new node
	nodeManager.GetAliveNodes()["node2"] = &node.Info{ID: "node2"}
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 50, s.operatorController.OperatorSize())
	require.Equal(t, 50, s.controller.replicationDB.GetSchedulingSize())
	require.Equal(t, 50, s.controller.replicationDB.GetReplicatingSize())
	for _, span := range s.controller.replicationDB.GetTasksBySchemaID(1) {
	...
	require.Equal(t, 100, s.controller.replicationDB.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 0, s.controller.replicationDB.GetTaskSizeByNodeID("node2"))
	...
	require.Equal(t, 0, s.controller.replicationDB.GetSchedulingSize())
...
func TestBalanceGlobalUneven(t *testing.T) {
	...
	require.Equal(t, 0, s.operatorController.OperatorSize())
	require.Equal(t, 100, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, 50, s.controller.replicationDB.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 50, s.controller.replicationDB.GetTaskSizeByNodeID("node2"))

	// add new node
	nodeManager.GetAliveNodes()["node3"] = &node.Info{ID: "node3"}
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 33, s.operatorController.OperatorSize())
	require.Equal(t, 33, s.controller.replicationDB.GetSchedulingSize())
	require.Equal(t, 67, s.controller.replicationDB.GetReplicatingSize())
	for _, span := range s.controller.replicationDB.GetTasksBySchemaID(1) {
		if op := s.operatorController.GetOperator(span.ID); op != nil {
			_, ok := op.(*operator.MoveDispatcherOperator)
			require.True(t, ok)
...
func TestStoppedWhenMoving(t *testing.T) {
	...
	require.Equal(t, 2, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, 2, s.controller.replicationDB.GetTaskSizeByNodeID("node1"))
	// add new node
	nodeManager.GetAliveNodes()["node2"] = &node.Info{ID: "node2"}
	require.Equal(t, 0, s.controller.replicationDB.GetAbsentSize())
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 1, s.operatorController.OperatorSize())
	require.Equal(t, 1, s.controller.replicationDB.GetSchedulingSize())
	require.Equal(t, 1, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, 2, s.controller.replicationDB.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 0, s.controller.replicationDB.GetTaskSizeByNodeID("node2"))
	...
	require.Equal(t, 2, s.controller.replicationDB.GetAbsentSize())
	require.Equal(t, 0, s.controller.replicationDB.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 0, s.controller.replicationDB.GetTaskSizeByNodeID("node2"))
}
...
func TestFinishBootstrap(t *testing.T) {
	...
	s := NewControllerManager(cfID, 1, nil, nil, &mockThreadPool{},
	...
	require.Equal(t, 1, s.controller.replicationDB.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 1, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, 0, s.controller.replicationDB.GetSchedulingSize())
	require.NotNil(t, s.controller.replicationDB.GetTaskByID(dispatcherID2))
...
func TestBalanceUnEvenTask(t *testing.T) {
	...
	require.Equal(t, 4, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, 0, s.controller.replicationDB.GetSchedulingSize())
	...
	nodeManager.GetAliveNodes()["node3"] = &node.Info{ID: "node3"}
	s.schedulerController.GetScheduler(scheduler.BalanceScheduler).Execute()
	require.Equal(t, 3, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, 1, s.operatorController.OperatorSize())
	// still on the primary node
	require.Equal(t, 2, s.controller.GetTaskSizeByNodeID("node1"))
	require.Equal(t, 2, s.controller.GetTaskSizeByNodeID("node2"))
	require.Equal(t, 0, s.controller.GetTaskSizeByNodeID("node3"))
	...
	require.Equal(t, 1, s.controller.GetTaskSizeByNodeID("node3"))
}
...
func TestSplitTableWhenBootstrapFinished(t *testing.T) {
	...
	require.Equal(t, 4, s.controller.replicationDB.GetAbsentSize())
...
func TestDynamicSplitTableBasic(t *testing.T) {
	...
	replicas := s.controller.replicationDB.GetReplicating()
	require.Equal(t, 2, s.controller.replicationDB.GetReplicatingSize())

	for _, task := range replicas {
		for cnt := 0; cnt < replica.HotSpanScoreThreshold; cnt++ {
			s.controller.replicationDB.UpdateStatus(task, &heartbeatpb.TableSpanStatus{
				ID:                 task.ID.ToPB(),
				ComponentStatus:    heartbeatpb.ComponentState_Working,
				CheckpointTs:       10,
	...
	require.Equal(t, 2, s.controller.replicationDB.GetSchedulingSize())
...
func TestDynamicMergeAndSplitTable(t *testing.T) {
	...
	replicas := s.controller.replicationDB.GetReplicating()
	require.Equal(t, totalTables*3-1, s.controller.replicationDB.GetReplicatingSize())
	...
	require.Equal(t, 7, s.controller.replicationDB.GetAbsentSize())
}
...
func TestDynamicMergeTableBasic(t *testing.T) {
	...
	victimExpected := 2
	replicas := s.controller.replicationDB.GetReplicating()
	require.Equal(t, expected+victimExpected, s.controller.replicationDB.GetReplicatingSize())
	...
	require.Equal(t, victimExpected, s.controller.replicationDB.GetReplicatingSize())
	require.Equal(t, expected, s.controller.replicationDB.GetSchedulingSize())
	...
	require.Equal(t, totalTables-1, s.controller.replicationDB.GetAbsentSize())
	...
	require.Equal(t, 0, s.controller.replicationDB.GetReplicatingSize())
	...
	require.Equal(t, totalTables, s.controller.replicationDB.GetAbsentSize())
}
