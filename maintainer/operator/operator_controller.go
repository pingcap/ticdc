...
func (oc *Controller) pollQueueingOperator() (
	...
	if op.IsFinished() {
		...
		metrics.OperatorCount.WithLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), op.Type(), common.StringMode(oc.mode)).Dec()
		metrics.OperatorDuration.WithLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), op.Type(), common.StringMode(oc.mode)).Observe(time.Since(item.CreatedAt).Seconds())
...
func (oc *Controller) pushOperator(op operator.Operator[common.DispatcherID, *heartbeatpb.TableSpanStatus]) {
	...
	metrics.OperatorCount.WithLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), op.Type(), common.StringMode(oc.mode)).Inc()
	metrics.TotalOperatorCount.WithLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), op.Type(), common.StringMode(oc.mode)).Inc()
}
...
func (oc *Controller) Close() {
	...
	for _, opType := range opTypes {
		metrics.OperatorCount.DeleteLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), opType, common.StringMode(oc.mode))
		metrics.TotalOperatorCount.DeleteLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), opType, common.StringMode(oc.mode))
		metrics.OperatorDuration.DeleteLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), opType, common.StringMode(oc.mode))
	}
}
