...
func TestHandleBootstrapRequestReplacesMismatchedTableTriggerDispatcher(t *testing.T) {
	mc := messaging.NewMockMessageCenter()
	appcontext.SetService(appcontext.DefaultPDClock, pdutil.NewClock4Test())
	appcontext.SetService(appcontext.MessageCenter, mc)
	heartbeatCollector := dispatchermanager.NewHeartBeatCollector(node.ID("receiver"))
	...
	configBytes, err := json.Marshal(newBootstrapResponseTestChangefeedConfig(cfID))
	require.NoError(t, err)
	err = orchestrator.handleBootstrapRequest(node.ID("self"), &heartbeatpb.MaintainerBootstrapRequest{
		ChangefeedID:                  cfID.ToPB(),
		Config:                        configBytes,
		StartTs:                       200,
		TableTriggerEventDispatcherId: newTriggerID.ToPB(),
		MaintainerEpoch:               2,
	})
	require.NoError(t, err)
	...
	require.Nil(t, bootstrapResponse.Err)
	require.Equal(t, uint64(2), bootstrapResponse.MaintainerEpoch)
	require.Equal(t, uint64(100), bootstrapResponse.CheckpointTs)
	require.Equal(t, newTriggerID, manager.GetTableTriggerEventDispatcher().GetId())
	...
	err = orchestrator.handlePostBootstrapRequest(node.ID("self"), &heartbeatpb.MaintainerPostBootstrapRequest{
		ChangefeedID:                  cfID.ToPB(),
		TableTriggerEventDispatcherId: newTriggerID.ToPB(),
		MaintainerEpoch:               2,
	})
	require.NoError(t, err)
	postBootstrapResponseMsg := <-mc.GetMessageChannel()
	postBootstrapResponse := postBootstrapResponseMsg.Message[0].(*heartbeatpb.MaintainerPostBootstrapResponse)
	require.Nil(t, postBootstrapResponse.Err)
	require.Equal(t, newTriggerID, common.NewDispatcherIDFromPB(postBootstrapResponse.TableTriggerEventDispatcherId))
}
