...
type BlockEventWithID struct {
	id         common.DispatcherID
	blockEvent commonEvent.BlockEvent
}

func newBlockEventWithID(id common.DispatcherID, blockEvent commonEvent.BlockEvent) BlockEventWithID {
	return BlockEventWithID{
		id:         id,
		blockEvent: blockEvent,
	}
}

func NewBlockEventDynamicStream() dynstream.DynamicStream[int, common.DispatcherID, BlockEventWithID, Dispatcher, *BlockEventTaskHandler] {
	ds := dynstream.NewParallelDynamicStream(
		&BlockEventTaskHandler{})
	ds.Start()
	return ds
}

type BlockEventTaskHandler struct{}

func (h *BlockEventTaskHandler) Path(blockEventWithID BlockEventWithID) common.DispatcherID {
	return blockEventWithID.id
}

func (h *BlockEventTaskHandler) Handle(dispatcher Dispatcher, blockEvents ...BlockEventWithID) bool {
	if len(blockEvents) > 1 {
		panic("invalid block events")
	}
	blockEvent := blockEvents[0]
	dispatcher.DealWithBlockEvent(blockEvent.blockEvent)
	return false
}

func (h *BlockEventTaskHandler) GetSize(blockEventWithID BlockEventWithID) int   { return 0 }
func (h *BlockEventTaskHandler) IsPaused(blockEventWithID BlockEventWithID) bool { return false }
func (h *BlockEventTaskHandler) GetArea(path common.DispatcherID, dest Dispatcher) int {
	return 0
}

func (h *BlockEventTaskHandler) GetTimestamp(event BlockEventWithID) dynstream.Timestamp {
	return 0
}

func (h *BlockEventTaskHandler) GetType(event BlockEventWithID) dynstream.EventType {
	return dynstream.EventType{DataGroup: 1, Property: dynstream.NonBatchable}
}

func (h *BlockEventTaskHandler) OnDrop(event BlockEventWithID) interface{} {
	return nil
}
