...
func TestNewDispatcherMessageRetryPolicy(t *testing.T) {
	localServerID := node.ID("local")
	remoteServerID := node.ID("remote")

	heartbeatMsg := messaging.NewSingleTargetMessage(
		remoteServerID,
		messaging.EventServiceTopic,
		&commonEvent.DispatcherHeartbeat{},
	)
	dispatcherReqMsg := messaging.NewSingleTargetMessage(
		remoteServerID,
		messaging.EventServiceTopic,
		&messaging.DispatcherRequest{
			DispatcherRequest: &eventpb.DispatcherRequest{
				ActionType: eventpb.ActionType_ACTION_TYPE_RESET,
			},
		},
	)
	localDispatcherReqMsg := messaging.NewSingleTargetMessage(
		localServerID,
		messaging.EventServiceTopic,
		&messaging.DispatcherRequest{
			DispatcherRequest: &eventpb.DispatcherRequest{
	...
	heartbeat := newDispatcherMessage(localServerID, heartbeatMsg)
	require.Equal(t, dispatcherMessageRetryWithQuota, heartbeat.retryMode)
	require.Equal(t, 1, heartbeat.remainingRetries)

	dispatcherReq := newDispatcherMessage(localServerID, dispatcherReqMsg)
	require.Equal(t, dispatcherMessageRetryWithQuota, dispatcherReq.retryMode)
	require.Equal(t, commonMsgRetryQuota, dispatcherReq.remainingRetries)

	localDispatcherReq := newDispatcherMessage(localServerID, localDispatcherReqMsg)
	require.Equal(t, dispatcherMessageRetryUntilSuccess, localDispatcherReq.retryMode)
	require.Equal(t, 0, localDispatcherReq.remainingRetries)
}
