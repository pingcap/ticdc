...
func TestUpdateScanMaxTsIgnoresDispatchersWithoutResolved(t *testing.T) {
	...
	addDispatcher := func(id common.DispatcherID, checkpoint time.Time, eligible bool) {
		...
		stat.hasReceivedResolvedTsOnce.Store(eligible)
...
func TestUpdateScanMaxTsReusesLastBaseWhenEligibleEmpty(t *testing.T) {
	...
	stat1.hasReceivedResolvedTsOnce.Store(true)
	...
	stat2.hasReceivedResolvedTsOnce.Store(true)
	...
	stat1.hasReceivedResolvedTsOnce.Store(false)
	stat2.hasReceivedResolvedTsOnce.Store(false)
