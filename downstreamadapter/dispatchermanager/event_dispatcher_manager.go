...
func (e *EventDispatcherManager) closeAllDispatchers() {
	...
	e.dispatcherMap.ForEach(func(id common.DispatcherID, d dispatcher.EventDispatcher) {
		...
		_, ok := d.(*dispatcher.Dispatcher).TryClose()
		if !ok {
			leftToCloseDispatchers = append(leftToCloseDispatchers, d.(*dispatcher.Dispatcher))
		} else {
			// Remove should be called after dispatcher is closed
			d.(*dispatcher.Dispatcher).Remove()
		}
	})
...
func (e *EventDispatcherManager) InitalizeTableTriggerEventDispatcher(schemaInfo []*heartbeatpb.SchemaInfo) error {
	...
	appcontext.GetService[*eventcollector.EventCollector](appcontext.EventCollector).AddDispatcher(e.tableTriggerEventDispatcher, e.sinkQuota, e.config.BDRMode, false)
	// redo
	if e.redoManager.Enabled() {
		appcontext.GetService[*eventcollector.EventCollector](appcontext.EventCollector).AddDispatcher(e.redoTableTriggerEventDispatcher, e.redoQuota, e.config.BDRMode, true)
	}
...
func (e *EventDispatcherManager) newDispatchers(infos []dispatcherCreateInfo, removeDDLTs bool) error {
	...
	for idx, id := range dispatcherIds {
		...
		if d.IsTableTriggerEventDispatcher() {
			...
			appcontext.GetService[*eventcollector.EventCollector](appcontext.EventCollector).AddDispatcher(d, e.sinkQuota, e.config.BDRMode, false)
			if e.redoManager.Enabled() {
				appcontext.GetService[*eventcollector.EventCollector](appcontext.EventCollector).AddDispatcher(rd, e.redoQuota, e.config.BDRMode, true)
			}
		}
...
func (e *EventDispatcherManager) aggregateDispatcherHeartbeats(needCompleteStatus bool) *heartbeatpb.HeartBeatRequest {
	...
	seq := e.dispatcherMap.ForEach(func(id common.DispatcherID, d dispatcher.EventDispatcher) {
		dispatcherItem := d.(*dispatcher.Dispatcher)
