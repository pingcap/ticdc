...
func (h *MergeDispatcherRequestHandler) Handle(dispatcherManager *DispatcherManager, reqs ...MergeDispatcherRequest) bool {
	...
	// Journal only an admitted request. Persisting a stale maintainer's merge would
	// make the next bootstrap restore an operation that the epoch fence rejected.
