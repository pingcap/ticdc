...
func defaultDDLKinds() []ddlKind {
	...
	return []ddlKind{
		...
		// Do not include RECOVER TABLE in the default CDC random DDL set.
		// It restores table data and schema from TiDB's local DDL history / GC
		// snapshot state, so executing the same text on the downstream can recover
		// a different historical table when names are reused. The recovered rows are
		// not emitted as new DML either, making the operation unsafe for this suite.
		{
