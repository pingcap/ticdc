...
#   3) start TiCDC (3 captures) and create changefeed with scheduler settings.
#   4) random_ddl_test_runner workload: concurrent random DML + random DDL on upstream.
...
#
# Notes:
#   - Storage sink: pre-create directories to avoid path creation races under multi-capture runs.
#
...
#     |-> cdc_cli_changefeed create (scheduler enabled)
#     |-> consumer (kafka/storage/pulsar) [optional]
#     |-> random_ddl_test_runner --phase workload
...
RUN_CONVERGE_TIMEOUT=${RUN_CONVERGE_TIMEOUT:-}
if [ -z "$RUN_CONVERGE_TIMEOUT" ]; then
	if [ "$RUN_PROFILE" == "weekly" ]; then
		RUN_CONVERGE_TIMEOUT="120m"
	else
		RUN_CONVERGE_TIMEOUT="30m"
	fi
fi
	...
	cat >"$WORK_DIR/runner_config.json" <<EOF
{
  ...
  "verify": {
    ...
    "no_advance_hard": "5m",
    "converge_wait": "20s",
    "converge_timeout": "$RUN_CONVERGE_TIMEOUT",
