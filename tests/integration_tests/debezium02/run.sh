	...
	localhost:8083/connectors/ --data-binary @- <<EOF
{
  ...
  "config": {
    ...
    "binary.handling.mode": "base64-url-safe",
...
cdc_cli_changefeed create -c test --sink-uri="kafka://127.0.0.1:9092/output_ticdc?protocol=debezium&kafka-version=2.4.0&debezium-binary-handling-mode=base64-url-safe&debezium-decimal-handling-mode=string&debezium-bigint-unsigned-handling-mode=long" --config "$CUR/changefeed.toml"
