...
# Random DDL+DML weekly smoke test (single capture).
...
function prepare_sink() {
	local topic="ticdc-weekly-rand-single-$RANDOM"
	case $SINK_TYPE in
	kafka)
		...
		;;
	storage)
		SINK_URI="file://$WORK_DIR/storage_test/$topic?flush-interval=5s&protocol=csv"
		;;
	pulsar)
