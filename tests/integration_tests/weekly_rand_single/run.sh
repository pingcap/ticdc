...
#   3) start TiCDC (single capture) and create changefeed (optionally start MQ/storage consumer).
#   4) random_ddl_test_runner workload: run concurrent random DML + random DDL against upstream only.
