DROP DATABASE IF EXISTS test_100;
CREATE DATABASE test_100;
USE test_100;
CREATE TABLE t1 (a INT, b INT, c DECIMAL NULL, d FLOAT NULL DEFAULT 100.0, e DECIMAL NOT NULL, site_code VARCHAR(64) NOT NULL DEFAULT '', PRIMARY KEY (a, c, d, e, site_code));
INSERT INTO t1 VALUES (1, 2, '100'), (2, 3, '100'), (12, 13, '100');

DROP DATABASE IF EXISTS test_200;
CREATE DATABASE test_200;
USE test_200;
CREATE TABLE t1 (a INT, b INT, c DECIMAL NULL, d FLOAT NULL DEFAULT 200.0, e DECIMAL NOT NULL, site_code VARCHAR(64) NOT NULL DEFAULT '', PRIMARY KEY (a, c, d, e, site_code));
INSERT INTO t1 VALUES (1, 2, '200'), (2, 3, '200'), (12, 13, '200');

DROP DATABASE IF EXISTS test_300;
CREATE DATABASE test_300;
USE test_300;
CREATE TABLE t1 (a INT, b INT, c DECIMAL NULL, d FLOAT NULL DEFAULT 300.0, e DECIMAL NOT NULL, site_code VARCHAR(64) NOT NULL DEFAULT '', PRIMARY KEY (a, c, d, e, site_code));
INSERT INTO t1 VALUES (1, 2, '300'), (2, 3, '300'), (12, 13, '300');

DROP DATABASE IF EXISTS test_400;
CREATE DATABASE test_400;
USE test_400;
CREATE TABLE t1 (a INT, b INT, c DECIMAL NULL, d FLOAT NULL DEFAULT 400.0, e DECIMAL NOT NULL, site_code VARCHAR(64) NOT NULL DEFAULT '', PRIMARY KEY (a, c, d, e, site_code));
INSERT INTO t1 VALUES (1, 2, '400'), (2, 3, '400'), (12, 13, '400');

DROP DATABASE IF EXISTS test_500;
CREATE DATABASE test_500;
USE test_500;
CREATE TABLE t1 (a INT, b INT, c DECIMAL NULL, d FLOAT NULL DEFAULT 500.0, e DECIMAL NOT NULL, site_code VARCHAR(64) NOT NULL DEFAULT '', PRIMARY KEY (a, site_code));
INSERT INTO t1 VALUES (1, 2, '500'), (2, 3, '500'), (12, 13, '500');
