...
func (r *regionFailureReporter) handleError(ctx context.Context, errInfo regionErrorInfo) error {
	...
	case *eventError:
		...
		if notLeader := innerErr.GetNotLeader(); notLeader != nil {
			...
			r.upstream.regionCache.UpdateLeader(errInfo.verID, notLeader.GetLeader(), errInfo.rpcCtx.AccessIdx)
		...
		if innerErr.GetCongested() != nil {
			...
			r.scheduleRegionRequest(ctx, errInfo.regionInfo, TaskLowPrior)
			return nil
		}
	...
	case *getStoreErr:
		...
		r.upstream.regionCache.OnSendFail(bo, errInfo.rpcCtx, true, err)
		r.scheduleRangeRequest(ctx, errInfo.span, errInfo.subscribedSpan, errInfo.filterLoop, TaskHighPrior)
		return nil
	case *storeStreamErr:
		metricStoreSendRequestErr.Inc()
		bo := tikv.NewBackoffer(ctx, tikvRequestMaxBackoff)
		r.upstream.regionCache.OnSendFail(bo, errInfo.rpcCtx, regionScheduleReload, err)
		r.scheduleRegionRequest(ctx, errInfo.regionInfo, TaskHighPrior)
		return nil
