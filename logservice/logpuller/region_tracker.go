...
// regionTracker is the regionRequestWorker-local state registry.
//
// It tracks only the regions sent by one worker. Normal scan request cleanup is
// owned by regionFeedState state transitions, so the tracker only indexes and
// removes states. The overwrite path aborts the old state's scan explicitly
// because no caller will transition that old state afterwards.
...
// Track records a region after the worker has picked a scan request from
// requestCache and before the request is sent to TiKV.
//
// If the same region is tracked again, the previous state's unfinished scan
// request is aborted before it can leak quota.
...
// Get returns the tracked region state for events received from TiKV.
//
// A nil return means the event belongs to a stale region, for example after a
// deregister request or stream cleanup.
...
// RemoveRegion removes one tracked region.
...
// RemoveSubscription removes all tracked regions for a deregistered
// subscription.
...
// Drain removes all tracked regions when the worker stream is closed.
