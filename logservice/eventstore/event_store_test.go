...
func TestPrepareValueForWrite(t *testing.T) {
	...
	testCases := []struct {
		...
		wantDirectWrite     bool
		wantCompressionType CompressionType
	}{
		{
			name:                "fast path",
			...
			wantDirectWrite:     true,
			wantCompressionType: CompressionNone,
		},
		{
			name:                "compression only",
			value:               bytes.Repeat([]byte("a"), compressionThreshold+64),
			enableCompression:   true,
			enableEncryption:    false,
			wantDirectWrite:     false,
			wantCompressionType: CompressionZSTD,
		},
		{
		...
		{
			name:                "compression then encryption",
			value:               bytes.Repeat([]byte("b"), compressionThreshold+64),
			enableCompression:   true,
			enableEncryption:    true,
			wantDirectWrite:     false,
			wantCompressionType: CompressionZSTD,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			rawBufSeed := []byte("keep-raw")
			compressionBufSeed := []byte("keep-compression")

			var spy *spyEncryptionManager
			var encryptionManager encryption.EncryptionManager
			if tc.enableEncryption {
				spy = &spyEncryptionManager{}
				encryptionManager = spy
			}

			store := &eventStore{
				compressionThreshold:  compressionThreshold,
				enableZstdCompression: tc.enableCompression,
				encryptionManager:     encryptionManager,
			}

			prepared, err := store.prepareValueForWrite(
				encoder, 42, &kv, append([]byte(nil), rawBufSeed...), append([]byte(nil), compressionBufSeed...),
			)
			require.NoError(t, err)
			require.Equal(t, tc.wantDirectWrite, prepared.directWrite)
			require.Equal(t, tc.wantCompressionType, prepared.compressionType)

			if tc.wantDirectWrite {
				require.Nil(t, prepared.value)
				require.Equal(t, rawBufSeed, prepared.rawBuf)
				require.Equal(t, compressionBufSeed, prepared.compressionBuf)
				require.Equal(t, kv.GetSize(), prepared.valueBytesAfter)
				return
			}

			transformed := prepared.value
			if tc.enableEncryption {
				require.Equal(t, 1, spy.encryptCalls)
				transformed, err = spy.DecryptData(context.Background(), 42, transformed)
				require.NoError(t, err)
			}
			if tc.wantCompressionType == CompressionZSTD {
				transformed, err = decoder.DecodeAll(transformed, nil)
				require.NoError(t, err)
			}
			require.Equal(t, kv.Encode(), transformed)
			require.Len(t, prepared.rawBuf, 0)
		})
	}
}
