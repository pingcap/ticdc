...
func mustUpdateCapture(
	...
	err = state.Update(util.NewEtcdKey(etcd.GetEtcdKeyCaptureInfo(state.ClusterID, captureID)), data, false)
	require.NoError(t, err)
}
...
func mustDeleteCapture(t *testing.T, state *GlobalReactorState, captureID config.CaptureID) {
	...
	err := state.Update(util.NewEtcdKey(etcd.GetEtcdKeyCaptureInfo(state.ClusterID, captureID)), nil, false)
	require.NoError(t, err)
}
