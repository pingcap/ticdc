...
// ValidateAndParseRedactMode validates the input string and returns the parsed redact mode.
// Returns an error if the input is empty or not a valid redaction mode.
// Valid inputs: "off", "on", "marker" (case-insensitive), plus aliases like "true"/"false", "enable"/"disable", "1"/"0".
// Note: Empty string returns an error (use for API validation). For config parsing where empty means default OFF, empty is valid.
func ValidateAndParseRedactMode(input string) (string, error) {
	if strings.TrimSpace(input) == "" {
		return "", fmt.Errorf("missing required field 'redact_mode': must be 'off', 'on', or 'marker'")
	}
	parsed := parseRedactMode(input)
	if !isValidRedactMode(input, parsed) {
