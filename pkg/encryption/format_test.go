...
func TestEncodeEncryptedDataInvalidKey(t *testing.T) {
	_, err := EncodeEncryptedData([]byte("payload"), "12")
	require.Error(t, err)
...
func TestEncodeDecodeEncryptedData(t *testing.T) {
	...
	encoded, err := EncodeEncryptedData(data, keyID)
	require.NoError(t, err)
	require.True(t, IsEncrypted(encoded))

	version, decodedKeyID, body, err := DecodeEncryptedData(encoded)
	require.NoError(t, err)
	require.Equal(t, VersionEncrypted, version)
