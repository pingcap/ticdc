...
func NewMemQuota(totalBytes uint64, comp string) *MemQuota {
	m := &MemQuota{
		totalBytes:       totalBytes,
		blockAcquireCond: sync.NewCond(&sync.Mutex{}),
		metricTotal:      metrics.MemoryQuota.WithLabelValues("total", comp),
		metricUsed:       metrics.MemoryQuota.WithLabelValues("used", comp),
	...
	go func() {
		...
		defer func() {
			timer.Stop()
			metrics.MemoryQuota.DeleteLabelValues("total", comp)
			metrics.MemoryQuota.DeleteLabelValues("used", comp)
		}()
		for {
			select {
			case <-timer.C:
