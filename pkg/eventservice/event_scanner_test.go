...
func TestDMLProcessor(t *testing.T) {
	...
	t.Run("CreateNewDMLProcessor", func(t *testing.T) {
		processor := newDMLProcessor(mockMounter, mockSchemaGetter, nil, false, common.DefaultMode, common.BlackHoleSinkType)
...
func TestDMLProcessorAppendRow(t *testing.T) {
	...
	t.Run("AppendUpdateRowWithUKChangeAndOutputRawChangeEvent", func(t *testing.T) {
		processor := newDMLProcessor(event.NewMounter(time.UTC, &integrity.Config{}), mockSchemaGetter, nil, true, common.DefaultMode, common.BlackHoleSinkType)
		// Generate a real update event that changes unique key using helper
		// This updates the unique key column 'a' from 'a1' to 'a1_new'
...
func TestEventMerger(t *testing.T) {
	...
	t.Run("MixedDMLAndDDLEvents", func(t *testing.T) {
		...
		tableInfo := ddlEvent1.TableInfo
		processor := newDMLProcessor(mounter, mockSchemaGetter, nil, false, common.DefaultMode, common.BlackHoleSinkType)
...
func TestTxnEventSplit(t *testing.T) {
	...
	t.Run("SplitByRows", func(t *testing.T) {
		...
		txn, err := newTxnEvent(batchDML, dispatcherID, tableID, tableInfo, 100, 110, true, common.BlackHoleSinkType)
		require.NoError(t, err)
		// Set max rows to 2
