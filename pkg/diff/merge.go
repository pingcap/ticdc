...
func (r RowDatas) Less(i, j int) bool {
	for _, col := range r.OrderKeyCols {
		...
		if !ok {
			log.Fatal("data don't have column", zap.String("column", col.Name.O), // skip-redaction: column name only, not data zap.Reflect("data", r.Rows[i].Data))
		}
		col2, ok := r.Rows[j].Data[col.Name.O]
		if !ok {
			log.Fatal("data don't have column", zap.String("column", col.Name.O), // skip-redaction: column name only, not data zap.Reflect("data", r.Rows[j].Data))
		}
		...
		if err1 != nil {
			log.Fatal("convert string to float failed", zap.String("column", col.Name.O), zap.String("data", redact.Value(strData1)), zap.Error(err1))
		}
		num2, err2 := strconv.ParseFloat(strData2, 64)
		if err2 != nil {
			log.Fatal("convert string to float failed", zap.String("column", col.Name.O), zap.String("data", redact.Value(strData2)), zap.Error(err2))
		}
