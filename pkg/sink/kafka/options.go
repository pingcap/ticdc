...
func adjustOptions(
	...
	if exists {
		// make sure that producer's `MaxMessageBytes` is not larger than topic's `max.message.bytes`.
	...
	// TiCDC need to make sure that the producer's `MaxMessageBytes` won't be larger than
