...
func TestCompleteOptions(t *testing.T) {
	...
	err = options.Apply(commonType.NewChangefeedID4Test(commonType.DefaultKeyspace, "test"), sinkURI, config.GetDefaultReplicaConfig().Sink)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	options = NewOptions()
	err = options.Apply(commonType.NewChangefeedID4Test(commonType.DefaultKeyspace, "test"),
...
func TestClientID(t *testing.T) {
	...
	for _, tc := range testCases {
		id, err := NewKafkaClientID(tc.addr,
			commonType.NewChangefeedID4Test(commonType.DefaultKeyspace, tc.changefeedID), tc.configuredID)
		if tc.hasError {
			require.Error(t, err)
		} else {
			require.NoError(t, err)
...
func TestConfigurationCombinations(t *testing.T) {
	...
	for _, a := range combinations {
		...
		changefeed := commonType.NewChangefeedID4Test(commonType.DefaultKeyspace, "changefeed-test")
...
func TestMerge(t *testing.T) {
	...
	c := NewOptions()
	err = c.Apply(commonType.NewChangefeedID4Test(commonType.DefaultKeyspace, "test"), sinkURI, replicaConfig.Sink)
	require.NoError(t, err)
