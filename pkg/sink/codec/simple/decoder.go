...
func (d *Decoder) HasNext() (common.MessageType, bool) {
	...
	if err != nil {
		log.Panic("decoder unmarshal failed", zap.Any("value", d.value), zap.Error(err)) // skip-redaction: raw encoded message bytes, not human-readable
	}
	d.msg = m
	d.value = nil
...
func formatValue(value any, ft types.FieldType) any {
	...
	case mysql.TypeTimestamp:
		...
		if err != nil {
			log.Panic("invalid column value for time", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
		}
	case mysql.TypeEnum:
		var v uint64
		...
		case string:
			...
			if err != nil {
				log.Panic("invalid column value for enum", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
			}
		case int64:
			v = uint64(val)
		}
	...
	case mysql.TypeSet:
		...
		case string:
			v, err = strconv.ParseUint(val, 10, 64)
			if err != nil {
				log.Panic("invalid column value for set", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
			}
		case int64:
			v = uint64(val)
		}
	...
	case mysql.TypeBlob, mysql.TypeTinyBlob, mysql.TypeMediumBlob, mysql.TypeLongBlob,
		...
		case string:
			if mysql.HasBinaryFlag(ft.GetFlag()) {
				...
				if err != nil {
					log.Panic("invalid column value for binary char", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
				}
			} else {
				value = []byte(val)
			}
	...
	case mysql.TypeLonglong:
		...
		case string:
			...
			if err != nil {
				log.Panic("cannot parse int64 value from string", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
			}
		}
	case mysql.TypeTiny, mysql.TypeShort, mysql.TypeInt24, mysql.TypeLong:
		...
		case int64:
			v = val
		default:
			log.Panic("invalid column value for int", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.String("type", fmt.Sprintf("%T", value)))
		}
		if mysql.HasUnsignedFlag(ft.GetFlag()) {
			value = uint64(v)
		} else {
	...
	case mysql.TypeFloat:
		...
		case string:
			...
			if err != nil {
				log.Panic("cannot parse float32 value from string", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
			}
			value = float32(v)
		}
	case mysql.TypeDouble:
		...
		case string:
			...
			if err != nil {
				log.Panic("cannot parse float64 value from string", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
			}
		}
	case mysql.TypeJSON:
		value, err = types.ParseBinaryJSONFromString(value.(string))
		if err != nil {
			log.Panic("invalid column value for json. Use zero json instead", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
		}
	case mysql.TypeNewDecimal:
		result := new(types.MyDecimal)
		err = result.FromString([]byte(value.(string)))
		if err != nil {
			log.Panic("invalid column value for decimal", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
		}
		// workaround the decimal `digitInt` field incorrect problem.
		bin, err := result.ToBin(ft.GetFlen(), ft.GetDecimal())
		if err != nil {
			log.Panic("convert decimal to binary failed", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
		}
		_, err = result.FromBin(bin, ft.GetFlen(), ft.GetDecimal())
		if err != nil {
			log.Panic("convert binary to decimal failed", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
		}
		value = result
	case mysql.TypeDuration:
		value, _, err = types.ParseDuration(types.DefaultStmtNoWarningContext, value.(string), ft.GetDecimal())
		if err != nil {
			log.Panic("invalid column value for duration.", zap.String("value", redact.Value(fmt.Sprintf("%v", value))))
		}
	...
	case mysql.TypeTiDBVectorFloat32:
		...
		if err != nil {
			log.Panic("cannot parse vector32 value from string.", zap.String("value", redact.Value(fmt.Sprintf("%v", value))), zap.Error(err))
		}
	default:
	}
	return value
