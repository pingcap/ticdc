...
func createDDLEvent(job *timodel.Job) *commonEvent.DDLEvent {
...
func TestEncodeCheckpoint(t *testing.T) {
	...
	for _, format := range []common.EncodingFormatType{
		...
		for _, compressionType := range []string{
			...
			_, err = enc.EncodeCheckpointEvent(uint64(checkpoint))
			require.NoError(t, err)
...
func TestEncodeDMLEnableChecksum(t *testing.T) {
	...
	for _, format := range []common.EncodingFormatType{
		...
		for _, compressionType := range []string{
			...
			_, err = enc.EncodeDDLEvent(createTableDDL)
			require.NoError(t, err)
...
func TestE2EPartitionTable(t *testing.T) {
	...
	job := helper.DDL2Job(`create table test.t(a int primary key, b int) partition by range (a) (
		partition p0 values less than (10),
		partition p1 values less than (20),
		partition p2 values less than MAXVALUE)`)
	require.NotNil(t, job)
	insertEvent := helper.DML2Event("test", "t", `insert into test.t values (1, 1)`, `insert into test.t values (11, 11)`, `insert into test.t values (21, 21)`)
	...
	for _, format := range []common.EncodingFormatType{
		...
		_, err = enc.EncodeDDLEvent(createDDLEvent(job))
		require.NoError(t, err)
		...
		for {
			event, ok := insertEvent.GetNextRow()
			if !ok {
				break
			}
			...
			err = enc.AppendRowChangedEvent(ctx, "", &commonEvent.RowEvent{
				...
				Event:          event,
				Callback:       insertEvent.PostFlush,
...
func TestEncodeDDLSequence(t *testing.T) {
	...
	dropDBEvent := helper.DDL2Job(`DROP DATABASE IF EXISTS test`)
	createDBDDLEvent := helper.DDL2Job(`CREATE DATABASE IF NOT EXISTS test`)
	helper.Tk().MustExec("use test")

	createTableDDLEvent := helper.DDL2Job("CREATE TABLE `TBL1` (`id` INT PRIMARY KEY AUTO_INCREMENT,`value` VARCHAR(255),`payload` VARCHAR(2000),`a` INT)")

	addColumnDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` ADD COLUMN `nn` INT")

	dropColumnDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` DROP COLUMN `nn`")

	changeColumnDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` CHANGE COLUMN `value` `value2` VARCHAR(512)")

	modifyColumnDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` MODIFY COLUMN `value2` VARCHAR(512) FIRST")

	setDefaultDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` ALTER COLUMN `payload` SET DEFAULT _UTF8MB4'a'")

	dropDefaultDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` ALTER COLUMN `payload` DROP DEFAULT")

	autoIncrementDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` AUTO_INCREMENT = 5")

	modifyColumnNullDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` MODIFY COLUMN `a` INT NULL")

	modifyColumnNotNullDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` MODIFY COLUMN `a` INT NOT NULL")

	addIndexDDLEvent := helper.DDL2Job("CREATE INDEX `idx_a` ON `TBL1` (`a`)")

	renameIndexDDLEvent := helper.DDL2Job("ALTER TABLE `TBL1` RENAME INDEX `idx_a` TO `new_idx_a`")

	indexVisibilityDDLEvent := helper.DDL2Job("ALTER TABLE TBL1 ALTER INDEX `new_idx_a` INVISIBLE")

	dropIndexDDLEvent := helper.DDL2Job("DROP INDEX `new_idx_a` ON `TBL1`")

	truncateTableDDLEvent := helper.DDL2Job("TRUNCATE TABLE TBL1")

	multiSchemaChangeDDLEvent := helper.DDL2Job("ALTER TABLE TBL1 ADD COLUMN `new_col` INT, ADD INDEX `idx_new_col` (`a`)")

	multiSchemaChangeDropDDLEvent := helper.DDL2Job("ALTER TABLE TBL1 DROP COLUMN `new_col`, DROP INDEX `idx_new_col`")

	renameTableDDLEvent := helper.DDL2Job("RENAME TABLE TBL1 TO TBL2")

	helper.Tk().MustExec("set @@tidb_allow_remove_auto_inc = 1")
	renameColumnDDLEvent := helper.DDL2Job("ALTER TABLE TBL2 CHANGE COLUMN `id` `id2` INT")

	partitionTableDDLEvent := helper.DDL2Job("ALTER TABLE TBL2 PARTITION BY RANGE (id2) (PARTITION p0 VALUES LESS THAN (10), PARTITION p1 VALUES LESS THAN (20))")

	addPartitionDDLEvent := helper.DDL2Job("ALTER TABLE TBL2 ADD PARTITION (PARTITION p2 VALUES LESS THAN (30))")

	dropPartitionDDLEvent := helper.DDL2Job("ALTER TABLE TBL2 DROP PARTITION p2")

	truncatePartitionDDLevent := helper.DDL2Job("ALTER TABLE TBL2 TRUNCATE PARTITION p1")

	reorganizePartitionDDLEvent := helper.DDL2Job("ALTER TABLE TBL2 REORGANIZE PARTITION p1 INTO (PARTITION p3 VALUES LESS THAN (40))")

	removePartitionDDLEvent := helper.DDL2Job("ALTER TABLE TBL2 REMOVE PARTITIONING")

	alterCharsetCollateDDLEvent := helper.DDL2Job("ALTER TABLE TBL2 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_bin")

	dropTableDDLEvent := helper.DDL2Job("DROP TABLE TBL2")
	...
	for _, format := range []common.EncodingFormatType{
		...
		for _, compressionType := range []string{
			...
			m, err := enc.EncodeDDLEvent(createDDLEvent(dropDBEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(createDBDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(createTableDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(addColumnDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(dropColumnDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(changeColumnDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(modifyColumnDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(setDefaultDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(dropDefaultDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(autoIncrementDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(modifyColumnNullDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(modifyColumnNotNullDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(addIndexDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(renameIndexDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(indexVisibilityDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(dropIndexDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(truncateTableDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(multiSchemaChangeDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(multiSchemaChangeDropDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(renameTableDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(renameColumnDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(partitionTableDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(addPartitionDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(dropPartitionDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(truncatePartitionDDLevent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(reorganizePartitionDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(removePartitionDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(alterCharsetCollateDDLEvent))
			require.NoError(t, err)
			...
			m, err = enc.EncodeDDLEvent(createDDLEvent(dropTableDDLEvent))
			require.NoError(t, err)
...
func TestEncodeDDLEvent(t *testing.T) {
	...
	createTableDDLEvent := helper.DDL2Job(createTableSQL)

	insertEvent := helper.DML2Event("test", "t", `insert into test.t values (1, "jack", "male", "jack@abc.com")`)

	renameTableDDLEvent := helper.DDL2Job(`rename table test.t to test.abc`)
	...
	for _, format := range []common.EncodingFormatType{
		...
		for _, compressionType := range []string{
			...
			_, err = enc.EncodeDDLEvent(createDDLEvent(createTableDDLEvent))
			require.NoError(t, err)
			...
			err = enc.AppendRowChangedEvent(context.Background(), "", insertEvent2)
			require.NoError(t, err)
...
func TestColumnFlags(t *testing.T) {
	...
	createTableDDLEvent := helper.DDL2Job(createTableDDL)
...
func TestEncodeIntegerTypes(t *testing.T) {
	...
	ddlEvent := helper.DDL2Job(createTableDDL)
...
// 	createPartitionTableDDL := helper.DDL2Job(`create table test.t(a int primary key, b int) partition by range (a) (
// 		partition p0 values less than (10),
// 		partition p1 values less than (20),
// 		partition p2 values less than MAXVALUE)`)
...
// 	err = enc.AppendRowChangedEvent(ctx, "", row)
// 	require.NoError(t, err)
...
// 	ddlEvent, insertEvent, updateEvent, deleteEvent := common.NewLargeEvent4Test(t, config.GetDefaultReplicaConfig())
...
func TestDDLMessageTooLarge(t *testing.T) {
	ddlEvent, _, _, _ := common.NewLargeEvent4Test(t, config.GetDefaultReplicaConfig())
...
func TestDMLMessageTooLarge(t *testing.T) {
	_, insertEvent, _, _ := common.NewLargeEvent4Test(t, config.GetDefaultReplicaConfig())
...
// 	ddlEvent, _, updateEvent, _ := common.NewLargeEvent4Test(t, config.GetDefaultReplicaConfig())
