...
func (e *mockEventEncoder) EncodeCheckpointEvent(uint64) (*codecCommon.Message, error) {
	return codecCommon.NewMsg(nil, nil), nil
}

func (e *mockEventEncoder) EncodeDDLEvent(*commonEvent.DDLEvent) (*codecCommon.Message, error) {
	return codecCommon.NewMsg(nil, []byte("ddl")), nil
}

func (e *mockEventEncoder) AppendRowChangedEvent(context.Context, string, *commonEvent.RowEvent) error {
	return nil
}

func (e *mockEventEncoder) Build() []*codecCommon.Message { return nil }
func (e *mockEventEncoder) Clean()                        {}

func getMockTableStatus(
	tableName string,
	tableID int64,
	totalPartition int32,
) (commonEvent.TopicPartitionKey, *commonEvent.RowEvent, *tableStatistic) {
	schema := "test"
...
func TestBootstrapWorker(t *testing.T) {
	...
	cfID := commonType.NewChangeFeedIDWithName("test", "default")
...
func TestUpdateTableStatistic(t *testing.T) {
	tableInfo1 := &commonType.TableInfo{
		TableName: commonType.TableName{
			Schema:      "test",
			Table:       "t1",
			TableID:     1,
	...
	row1 := &commonEvent.RowEvent{PhysicalTableID: tableInfo1.TableName.TableID, TableInfo: tableInfo1}
	tableStatistic := newTableStatistic(commonEvent.TopicPartitionKey{}, row1)

	// case 1: The tableStatistic should not be updated if the tableInfo is the same
	tableStatistic.update(row1, 1)
	require.Equal(t, tableInfo1, tableStatistic.tableInfo.Load().(*commonType.TableInfo))

	// case 2: The tableStatistic should be updated if the tableInfo is different
	tableInfo2 := &commonType.TableInfo{
		TableName: commonType.TableName{
			Schema:      "test",
			Table:       "t1",
			TableID:     tableInfo1.TableName.TableID,
			IsPartition: false,
		},
		UpdateTS: 2,
	}
	row2 := &commonEvent.RowEvent{PhysicalTableID: tableInfo2.TableName.TableID, TableInfo: tableInfo2}
	tableStatistic.update(row2, 1)
	require.Equal(t, tableInfo2, tableStatistic.tableInfo.Load().(*commonType.TableInfo))

	// case 3: The tableStatistic should be updated when rename table
	tableInfo3 := &commonType.TableInfo{
		TableName: commonType.TableName{
			Schema:      "test",
			Table:       "t2",
			TableID:     tableInfo1.TableName.TableID,
		...
		// Rename table may not bump UpdateTS, so keep it equal to the previous value.
		UpdateTS: tableInfo2.GetUpdateTS(),
	}
	row3 := &commonEvent.RowEvent{PhysicalTableID: tableInfo3.TableName.TableID, TableInfo: tableInfo3}
	tableStatistic.update(row3, 1)
	require.Equal(t, tableInfo3, tableStatistic.tableInfo.Load().(*commonType.TableInfo))
}
