...
func TestBinary(t *testing.T) {
	...
	_ = helper.DDL2Event("CREATE TABLE test.binary_columns(id INT AUTO_INCREMENT, c_binary binary(255), c_varbinary varbinary(255), t_tinyblob TINYBLOB, t_blob BLOB, t_mediumblob MEDIUMBLOB, t_longblob LONGBLOB, PRIMARY KEY (id))")

	insertSQL := `INSERT INTO test.binary_columns (c_binary, c_varbinary, t_tinyblob, t_blob, t_mediumblob, t_longblob)
				VALUES (
	...
	insertEvent := helper.DML2Event("test", "binary_columns", insertSQL)
	...
	m := encoder.Build()[0]
	path := filepath.Join("/Users/edison/Downloads", "ticdc-encoded.json")

	err = os.WriteFile(path, m.Value, 0644)
	require.NoError(t, err)
	log.Info("write data into the file", zap.String("path", path))
}
