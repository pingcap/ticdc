...
func (d *decoder) getCommitTs() uint64 {
	...
	if err != nil {
		log.Error("decode value failed", zap.Error(err), zap.String("value", redact.Value(fmt.Sprintf("%v", source))))
	}
...
func decodeColumn(value interface{}, colInfo *timodel.ColumnInfo, timeZone *time.Location) interface{} {
	...
	case mysql.TypeVarchar, mysql.TypeString, mysql.TypeVarString, mysql.TypeTinyBlob,
		...
		if mysql.HasBinaryFlag(colInfo.GetFlag()) {
			...
			if err != nil {
				log.Panic("decode value failed", zap.Error(err), zap.String("value", redact.Value(fmt.Sprintf("%v", value))))
			}
			return value
		}
