...
func (w *Writer) execDDL(event *commonEvent.DDLEvent) error {
	if w.cfg.DryRun {
		log.Info("Dry run DDL", zap.String("sql", util.RedactValue(event.GetDDLQuery())))
	...
	if timodel.ActionType(event.Type) == timodel.ActionExchangeTablePartition && w.cfg.EnableDDLTs {
		...
		if flag {
			log.Info("Skip Already Executed DDL", zap.String("sql", util.RedactValue(event.GetDDLQuery())))
			return nil
		}
	}
	...
	if w.cfg.HasVectorType {
		if newQuery := formatQuery(event.Query); newQuery != event.Query {
			log.Info("format ddl query", zap.String("newQuery", util.RedactValue(newQuery)), zap.String("query", util.RedactValue(event.Query)))
	...
	if err != nil {
		log.Error("Fail to ExecContext", zap.Any("err", err), zap.String("query", util.RedactValue(query)))
		if rbErr := tx.Rollback(); rbErr != nil {
			log.Error("Failed to rollback", zap.String("sql", util.RedactValue(event.GetDDLQuery())), zap.Error(err))
		}
		return err
	}
...
func (w *Writer) execDDLWithMaxRetries(event *commonEvent.DDLEvent) error {
	...
	return retry.Do(w.ctx, func() error {
		...
		if err != nil {
			if errors.IsIgnorableMySQLDDLError(err) {
				...
				log.Info("Execute DDL failed, but error can be ignored",
					zap.String("ddl", util.RedactValue(event.Query)),
					zap.Error(err))
			...
			if w.cfg.IsTiDB && ddlCreateTime != "" && errors.Cause(err) == mysql.ErrInvalidConn {
				log.Warn("Wait the asynchronous ddl to synchronize", zap.String("ddl", util.RedactValue(event.Query)), zap.String("ddlCreateTime", ddlCreateTime),
		...
		log.Info("Execute DDL succeeded",
			...
			zap.Uint64("commitTs", event.GetCommitTs()), zap.String("query", util.RedactValue(event.GetDDLQuery())),
			zap.String("ddl", util.RedactAny(event)))
		return nil
...
func (w *Writer) waitDDLDone(ctx context.Context, ddl *commonEvent.DDLEvent, ddlCreateTime string) error {
	...
	for {
		...
		case timodel.JobStateDone, timodel.JobStateSynced:
			log.Info("DDL replicate success", zap.String("ddl", util.RedactValue(ddl.Query)), zap.String("ddlCreateTime", ddlCreateTime))
			return nil
		...
		case timodel.JobStateRunning, timodel.JobStateQueueing:
			...
			case timodel.ActionAddIndex:
				log.Info("DDL is running downstream", zap.String("ddl", util.RedactValue(ddl.Query)), zap.String("ddlCreateTime", ddlCreateTime), zap.Any("ddlState", state))
				return nil
			}
		default:
			log.Warn("Unexpected DDL state, may not be found downstream", zap.String("ddl", util.RedactValue(ddl.Query)), zap.String("ddlCreateTime", ddlCreateTime), zap.Any("ddlState", state))
		...
		case <-ticker.C:
		case <-ticker1.C:
			log.Info("DDL is still running downstream, it blocks other DDL or DML events", zap.String("ddl", util.RedactValue(ddl.Query)), zap.String("ddlCreateTime", ddlCreateTime))
		}
	}
}
...
func (w *Writer) doQueryAsyncDDL(tableID int64, query string) (bool, error) {
	...
	log.Debug("query duration", zap.Any("duration", time.Since(start)), zap.String("query", util.RedactValue(query)))
	if err != nil {
