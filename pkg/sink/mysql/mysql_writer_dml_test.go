...
func TestGenerateBatchSQL(t *testing.T) {
	...
	sql, args, _ = writer.generateBatchSQL([]*commonEvent.DMLEvent{dmlInsertEvent, dmlInsertEvent2, dmlInsertEvent3})
	...
	start := time.Now()
	sql, args, _ = writer.generateBatchSQL([]*commonEvent.DMLEvent{dmlEvent})
...
func TestGenerateBatchSQLInUnSafeMode(t *testing.T) {
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlDeleteEvent, dmlUpdateEvent})
	require.Equal(t, 2, len(sql))
	require.Equal(t, 2, len(args))
	require.Equal(t, 2, len(rowTypes))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlDeleteEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlUpdateEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "INSERT INTO `test`.`t` (`id`,`name`) VALUES (?,?)", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlUpdateEvent, dmlDeleteEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlUpdateEvent, dmlUpdateEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlDeleteEvent, dmlInsertEvent, dmlUpdateEvent, dmlDeleteEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?),(?))", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlDeleteEvent, dmlInsertEvent, dmlUpdateEvent, dmlUpdateEvent2, dmlDeleteEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?),(?))", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlDeleteEvent, dmlInsertEvent2})
	require.Equal(t, 2, len(sql))
	require.Equal(t, 2, len(args))
	require.Equal(t, 2, len(rowTypes))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlUpdateEvent, dmlDeleteEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlUpdateEvent, dmlUpdateEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "INSERT INTO `test`.`t` (`id`,`name`) VALUES (?,?)", sql[0])
	...
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlDeleteEvent, dmlUpdateEvent})
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 17")
	dmlInsertEvent3 := helper.DML2Event("test", "t", "insert into t values (18, 'test')")
	sql, args, _ = writer.generateBatchSQLInUnSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlInsertEvent2, dmlInsertEvent3})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "INSERT INTO `test`.`t` (`id`,`name`) VALUES (?,?),(?,?),(?,?)", sql[0])
...
func TestGenerateBatchSQLInSafeMode(t *testing.T) {
	...
	dmlDeleteEvent = helper.DML2DeleteEvent("test", "t", "insert into t values (3, 'test')", "delete from t where id = 3")
	dmlInsertEvent = helper.DML2Event("test", "t", "insert into t values (3, 'test')")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlDeleteEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	require.Equal(t, []interface{}{int64(3)}, args[0])
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 4")
	dmlUpdateEvent, _ := helper.DML2UpdateEvent("test", "t", "insert into t values (4, 'test')", "update t set name = 'test4' where id = 4")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlUpdateEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "REPLACE INTO `test`.`t` (`id`,`name`) VALUES (?,?)", sql[0])
	require.Equal(t, []interface{}{int64(4), "test4"}, args[0])
	...
	dmlDeleteEvent = helper.DML2DeleteEvent("test", "t", "insert into t values (5, 'test5')", "delete from t where id = 5")
	dmlUpdateEvent, _ = helper.DML2UpdateEvent("test", "t", "insert into t values (5, 'test')", "update t set name = 'test5' where id = 5")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlUpdateEvent, dmlDeleteEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	require.Equal(t, []interface{}{int64(5)}, args[0])
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 6")
	dmlUpdateEvent2, _ := helper.DML2UpdateEvent("test", "t", "insert into t values (6, 'test6')", "update t set name = 'test7' where id = 6")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlUpdateEvent, dmlUpdateEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "REPLACE INTO `test`.`t` (`id`,`name`) VALUES (?,?)", sql[0])
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 8")
	dmlDeleteEvent2 := helper.DML2DeleteEvent("test", "t", "insert into t values (8, 'test8')", "delete from t where id = 8")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlDeleteEvent, dmlInsertEvent, dmlUpdateEvent, dmlDeleteEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 9")
	dmlDeleteEvent2 = helper.DML2DeleteEvent("test", "t", "insert into t values (9, 'test10')", "delete from t where id = 9")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlDeleteEvent, dmlInsertEvent, dmlUpdateEvent, dmlUpdateEvent2, dmlDeleteEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	dmlDeleteEvent = helper.DML2DeleteEvent("test", "t", "insert into t values (10, 'test')", "delete from t where id = 10")
	dmlInsertEvent2 = helper.DML2Event("test", "t", "insert into t values (10, 'test2')")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlDeleteEvent, dmlInsertEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "REPLACE INTO `test`.`t` (`id`,`name`) VALUES (?,?)", sql[0])
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 11")
	dmlDeleteEvent = helper.DML2DeleteEvent("test", "t", "insert into t values (11, 'test11')", "delete from t where id = 11")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlUpdateEvent, dmlDeleteEvent})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	require.Equal(t, []interface{}{int64(11)}, args[0])
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 12")
	dmlUpdateEvent2, _ = helper.DML2UpdateEvent("test", "t", "insert into t values (12, 'test12')", "update t set name = 'test13' where id = 12")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlUpdateEvent, dmlUpdateEvent2})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "REPLACE INTO `test`.`t` (`id`,`name`) VALUES (?,?)", sql[0])
	require.Equal(t, []interface{}{int64(12), "test13"}, args[0])
	...
	dmlDeleteEvent = helper.DML2DeleteEvent("test", "t", "insert into t values (14, 'test')", "delete from t where id = 14")
	dmlUpdateEvent, _ = helper.DML2UpdateEvent("test", "t", "insert into t values (15, 'test')", "update t set name = 'test15' where id = 15")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlDeleteEvent, dmlUpdateEvent})
	require.Equal(t, 2, len(sql))
	require.Equal(t, 2, len(args))
	require.Equal(t, 2, len(rowTypes))
	require.Equal(t, "DELETE FROM `test`.`t` WHERE (`id`) IN ((?))", sql[0])
	...
	helper.ExecuteDeleteDml("test", "t", "delete from t where id = 17")
	dmlInsertEvent3 := helper.DML2Event("test", "t", "insert into t values (18, 'test')")
	sql, args, _ = writer.generateBatchSQLInSafeMode([]*commonEvent.DMLEvent{dmlInsertEvent, dmlInsertEvent2, dmlInsertEvent3})
	require.Equal(t, 1, len(sql))
	require.Equal(t, 1, len(args))
	require.Equal(t, "REPLACE INTO `test`.`t` (`id`,`name`) VALUES (?,?),(?,?),(?,?)", sql[0])
	expected1 = []interface{}{int64(16), "test", int64(17), "test", int64(18), "test"}
