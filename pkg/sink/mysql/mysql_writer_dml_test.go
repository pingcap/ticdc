...
func TestGroupEventsByTable(t *testing.T) {
	...
	t.Run("events with multiple update timestamps should be sorted", func(t *testing.T) {
		helper := commonEvent.NewEventTestHelper(t)
		defer helper.Close()
		...
		// Add column to create new table version
		addColumnSQL := "alter table t add column age int;"
		job2 := helper.DDL2Job(addColumnSQL)
		require.NotNil(t, job2)

		// Create second event with new table version
		event2 := helper.DML2Event("test", "t", "insert into t values (2, 'test2', 20)")
		helper.ExecuteDeleteDml("test", "t", "delete from t where id = 2")

		// Add another column to create third table version
		addColumnSQL2 := "alter table t add column email varchar(50);"
		job3 := helper.DDL2Job(addColumnSQL2)
		require.NotNil(t, job3)

		// Create third event with newest table version
		event3 := helper.DML2Event("test", "t", "insert into t values (3, 'test3', 25, 'test3@example.com')")

		events := []*commonEvent.DMLEvent{event1, event2, event3}
		result := groupEventsByTable(events)

		// Should have 1 table group with 3 update timestamp groups
		require.Equal(t, 1, len(result))

		tableID := event1.GetTableID()
		tableGroups, exists := result[tableID]
		require.True(t, exists, "Table ID should exist in result")
		require.Equal(t, 3, len(tableGroups), "Should have 3 update timestamp groups")

		// Check that groups are sorted by update timestamp (ascending)
		updateTS1 := event1.TableInfo.GetUpdateTS()
		updateTS2 := event2.TableInfo.GetUpdateTS()
		updateTS3 := event3.TableInfo.GetUpdateTS()
		require.True(t, updateTS1 < updateTS2, "First group should have smaller update timestamp than second")
		require.True(t, updateTS2 < updateTS3, "Second group should have smaller update timestamp than third")

		// Each group should contain exactly 1 event
		require.Equal(t, 1, len(tableGroups[0]), "First group should have 1 event")
		require.Equal(t, 1, len(tableGroups[1]), "Second group should have 1 event")
		require.Equal(t, 1, len(tableGroups[2]), "Third group should have 1 event")
	})
}
