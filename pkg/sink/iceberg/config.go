...
func (c *Config) Apply(_ context.Context, sinkURI *url.URL, _ *config.SinkConfig) error {
	if sinkURI == nil {
		return cerror.ErrSinkURIInvalid.GenWithStackByArgs("sink uri is empty")
	}
	scheme := config.GetScheme(sinkURI)
	if scheme != "iceberg" && !config.IsStorageScheme(scheme) {
	...
	if c.WarehouseURI == "" {
		rawWarehouse := *sinkURI
		rawWarehouse.RawQuery = ""
		rawWarehouse.ForceQuery = false
		rawWarehouse.Fragment = ""
		if err := c.setWarehouse(rawWarehouse.String()); err != nil {
			return err
		}
	}
