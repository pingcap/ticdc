...
func (sp *SchemaParser) RemoveSchemaParserWithCondition(condition func(schemaPathKey cloudstorage.SchemaPathKey) bool) {
	sp.mu.Lock()
...
func NewTableDMLIdex() *TableDMLIdx {
	return &TableDMLIdx{
...
func (c *S3Consumer) downloadSchemaFiles(
	...
	for schemaPathKey, filePath := range newVersionPaths {
		eg.Go(func() error {
			...
			c.schemaParser.SetSchemaParser(schemaPathKey, filePath, parser)
			return nil
		})
	}
	if err := eg.Wait(); err != nil {
