...
func (w *writer) appendDDL(ddl *commonEvent.DDLEvent, offset kafka.Offset) {
	...
	if len(w.ddlCache) != 0 {
		lastDDL := w.ddlCache[len(w.ddlCache)-1]
		if ddl.GetCommitTs() < lastDDL.GetCommitTs() {
			log.Panic("DDL CommitTs fallback",
				zap.Uint64("commitTs", ddl.GetCommitTs()),
				zap.Uint64("lastDDLCommitTs", lastDDL.GetCommitTs()),
				zap.String("DDL", ddl.Query))
		}
	}

	w.ddlCache = append(w.ddlCache, ddl)
	log.Info("DDL event received", zap.Any("offset", offset),
